...
from tournament_selection import elite_selection_model, tournament_selection

START_POINT = [(1, 1)]
END_POINT = [(1, 1)]
POINTS = [(randint(0, 40), randint(0, 40)) for k in range(25)]
...
ga = GeneticAlgorithm(first_generation_generator, tournament_selection, stop_condition)
ga.run()
plt.show(block=True)